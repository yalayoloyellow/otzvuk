...
class Contacts {
  ...
  step(){
    // Расшатанный контакт садится обратно за пару секунд, и всё это время
    // норовит потерять пятно ещё раз. Частота таких потерь падает вместе с
    // расшатанностью — оттого треск и «успокаивается», а не обрывается.
    if (this.rasshat > .001){
      this.rasshat *= .99999;
      if (this.dreb <= 0 && Math.random() < this.rasshat * 1.4e-4){
        this.dreb = 1; this.faza = 0;
        this.dl = .002 + Math.random() * .03 * this.rasshat;
      }
    } else this.rasshat = 0;
    if (this.dreb > 0){
      this.faza += 1 / SR;
  ...
  soprot(){ return this.rasshat * 2200; }
