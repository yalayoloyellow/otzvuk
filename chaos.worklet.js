...
// ---- УРОВЕНЬ (ПОСТ) --------------------------------------------------------
// Не компрессор и не ограничитель. Это ручка уровня на том, чем коробку
// пишут: навёл микрофон, посмотрел на индикатор, убавил, если зашкаливает.
//
// Коробки различаются по отдаче в ДЕСЯТЬ раз по пику и на восемнадцать
// децибел по средней — замер по двенадцати сборкам, и разброс одинаков и
// ниже двухсот герц, и выше, так что дело не в полосе. Простой формулой это
// не описать: там совпадение резонанса корпуса со спектром самого прибора.
//
// Но УБАВЛЯТЬ можно, а ПРИБАВЛЯТЬ нельзя, и это не осторожность. yala
// слышит разные сборки примерно одинаково громкими, хотя разница в
// восемнадцать децибел реальна, — потому что в игре ручки меняют уровень на
// сопоставимые двенадцать децибел, и рука подстраивается сама, не думая.
// Автоматика, которая полезла бы поднимать тихие сборки вчетверо, стала бы
// драться с этой рукой. Поэтому предел усиления — единица: трогаем только
// то, что бьётся о потолок.
//
// Постоянные нарочно велики: две секунды на замер пика и три на движение.
// Внутри фразы уровень стоит намертво. Пик берётся ДО умножения, петли нет.
class Uroven {
  constructor(){ this.pik = .2; this.g = 1; }
  step(y){
    const a = y < 0 ? -y : y;
    if (a > this.pik) this.pik = a;
    else this.pik -= this.pik * this.zab;
    const cel = clamp(.78 / Math.max(this.pik, 1e-4), .22, 1);
    this.g += (cel - this.g) * this.hod;
    y *= this.g;
    // Мягкое колено вместо обрубания об единицу. До 0.8 не трогает вовсе —
    // наклон там ровно единичный, — а выше сводит к единице плавно. Это
    // последняя черта, за которой цифра просто ломается; медленный уровень
    // до неё почти не доводит, но «почти» тут не годится.
    const b = y < 0 ? -y : y;
    if (b > .8) y = (y < 0 ? -1 : 1) * (.8 + .2 * Math.tanh((b - .8) / .2));
    return y;
  }
}
Uroven.prototype.zab = 1 / (SR * 5.0);
Uroven.prototype.hod = 1 / (SR * 3.0);
...
class Chaos extends AudioWorkletProcessor {
  ...
  process(inputs, outputs){
    ...
    for (let s = 0; s < n; s++){
      ...
      // ПОСТ: уровень записи под эту коробку. Стоит здесь, а не в приборе:
      // прибор отдаёт сколько отдаёт, а сколько с него писать — вопрос не
      // к нему.
      y = clamp(this.uroven.step(y), -1, 1);
