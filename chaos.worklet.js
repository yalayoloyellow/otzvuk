...
// Ступени ручки ТЕМП и опорная точка привязки. Опора выведена из прежней
// свободной скорости: при такте 0.6 с и множителе ×1 речь идёт с тем же
// темпом, что была по умолчанию до привязки.
...
class Govorilka {
  ...
  step(ton, temp, povtor, trakt, pauza, period){
    ...
    // СКОРОСТЬ РЕЧИ привязана к качелям: прибор ускорился — говорилка
    // ускорилась вместе с ним, и слоги остаются на долях, а не плывут рядом.
    // Ручка ТЕМП при этом не задаёт скорость, а МНОЖИТ её ступенями —
    // ×0.25 … ×3, — как переключатель скорости на магнитофоне.
    const mnozh = TEMPY[clamp(Math.round(clamp(temp, 0, 1) * (TEMPY.length - 1)),
                              0, TEMPY.length - 1)];
    const skor = clamp(mnozh * OPORA / Math.max(period || .5, .02), .15, 8);
    if (this.zhdu > 0){
      this.zhdu -= RECH_DT;
