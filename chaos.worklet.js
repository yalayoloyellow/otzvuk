...
class Gromkost {
  ...
  step(y, cel, lim){
    ...
    if (this.videl && this.ms > 1e-9){
      ...
      // Ограничитель МАСКИРУЕТ промах: когда усиление сильно завышено, он
      // держит выход у потолка, и петля не видит, насколько всё лишнее. Из-за
      // этого после Tab на громкую сборку выход на уровень занимал четырнадцать
      // секунд. Поэтому его работа сверх обычных двух децибел входит в промах
      // напрямую: давит ограничитель — значит убавить.
      const lishnee = lim > 2 ? lim - 2 : 0;
      const promah = clamp(cel - this.lufs, -24, 24) - lishnee;
...
class Predel {
  ...
  // единственное, чего петля не может отменить: оно меняет не уровень, а
  // форму. Быстрое даёт накачку и дыхание, на которых эта музыка и держится;
  // медленное — прозрачность, когда ограничитель слышно только тем, что его
  // не слышно.
  nastroy(rel){
...
// Потолок с запасом на МЕЖОТСЧЁТНЫЕ пики: между двумя отсчётами сигнал
// поднимается выше их обоих, и цифро-аналоговое преобразование это покажет.
// Полтора децибела запаса держат true peak под −1 dBTP без отдельного счёта.
const POTOLOK = .80;
...
class Chaos extends AudioWorkletProcessor {
  ...
  process(inputs, outputs){
    ...
    for (let s = 0; s < n; s++){
      ...
      // звук поедет.
      // РУЧКИ ГРОМКОСТИ ЗДЕСЬ НЕТ, И БЫТЬ НЕ МОЖЕТ. Петля владеет выходным
      // уровнем, а значит съедает ЛЮБОЕ усиление, поставленное до неё:
      // поднял вдвое — петля убавила вдвое, на выходе то же самое. Замер
      // показал это дважды, на двух разных попытках сделать DRIVE. Опускать
      // потолок с добором — то же самое другими словами, и оно тоже
      // сократилось. Плотность держит COMP, уровень держит петля, и третьей
      // ручке между ними нечего делать.
      y = koleno(this.predel.step(y * this.grom.g));
      this.predel.nastroy(this.p.rel);
      this.grom.step(y, CEL_LUFS,
                     this.predel.g < 1 ? -20 * Math.log10(this.predel.g) : 0);
      this.proshY = y;
