...
class Build {
  constructor(semya){
    ...
    // УДАРНЫЙ КОНТУР — двойное Т в обратной связи транзистора. Ударишь по
    // нему импульсом — звенит и медленно гаснет; так сделан кик в 808-й и в
    // любой аналоговой драмке той эпохи, из тех же деталей, что уже на плате.
    //
    // Настроен он ЧУТЬ НИЖЕ резонанса коробки, и не по капризу: капсюль
    // отдаёт от 420 Гц и ниже не умеет физически, весь низ прибора идёт
    // через корпус. Контур на 50 Гц было бы просто нечем излучить. Собирая
    // такую коробку, настраивают контур под неё, а не наоборот.
    this.fUdar  = this.fKorp * m(.72, .95);
    this.Qudar  = m(46, 88);           // добротность при полной регенерации
    this.mzhUd  = m(.28, .52);         // насколько уводит частоту на большом ходу
...
// ---- УДАРНЫЙ КОНТУР --------------------------------------------------------
// Второй порядок с регенерацией. Ручка БАС поднимает обратную связь, то есть
// добротность: на нуле контура в схеме нет вовсе и звук прежний, дальше он
// звенит всё дольше — от короткого тука до полусекундного гула.
//
// Нелинейность тут не украшение. Транзистор на большом ходу подсаживается,
// эквивалентная ёмкость уплывает, и частота на старте ВЫШЕ, чем в хвосте.
// Этот увод и есть то, за что кик 808-й узнают с одного удара; без него
// выходит ровный синус, а не удар.
class Udarnik {
  constructor(sb){ this.sb = sb; this.x = 0; this.v = 0; }
  step(sila, tolchok){
    const s = clamp(sila, 0, 1);
    if (s < .002 && this.x === 0 && this.v === 0) return 0;
    const sb = this.sb;
    // Регенерация ведёт затухание: слабая связь — тук, сильная — гул.
    const Q = 2.5 + s * sb.Qudar;
    // На большом ходу контур ЖЁСТЧЕ: эквивалентная ёмкость подсаженного
    // транзистора меньше, и частота в начале удара ВЫШЕ, чем в хвосте. Знак
    // тут решает всё — с обратным получался кик, ползущий вверх, чего в
    // природе не бывает.
    const ω = 2 * Math.PI * sb.fUdar * (1 + sb.mzhUd * clamp(this.x * this.x, 0, 1));
    // Толчок это ИМПУЛЬС: он разом задаёт скорость, а не подмешивается к
    // силе на протяжении шага. Прежде он умножался на шаг времени, и размах
    // выходил в тысячи раз меньше нужного — контур звенел неслышно, а увод
    // частоты, который живёт на большом ходу, не появлялся вовсе.
    if (tolchok) this.v += tolchok * ω;
    this.v += dt * (-ω * ω * this.x - (ω / Q) * this.v);
    this.x += this.v * dt;
    if (!(this.x === this.x)){ this.x = 0; this.v = 0; }
    // Контур упирается в своё питание так же, как всё остальное на плате.
    return Math.tanh(this.x * 1.6) * s;
  }
}
...
class Device {
  ...
  step(p, utechka, navodka, kontakt, gls, ogib, petl){
    ...
    // УДАРНЫЙ КОНТУР бьётся по фронту акцента сетки — тем же ключом, что
    // коммутирует всё остальное на ударном шаге, — и от руки по корпусу.
    if (ritm > .5 && (this.ritmProsh || 0) <= .5) this.udarT += .55 * clamp(p.gryzn, 0, 1);
    this.ritmProsh = ritm;
    const tolchok = this.udarT; this.udarT = 0;
    const bas = this.udar.step(p.bas, tolchok);
    if (p.bas > .002 || bas !== 0){
      // Своя ветвь на ту же базу, как у генераторов и у микрофона.
      const Gb = 1 / (sb.Rsum0 * .35);
      Isum += bas * Vdd * .5 * Gb;
      Gsum += Gb;
    }
  ...
  // Удар ладонью бьёт и по корпусу, и по ударному контуру: рука встряхивает
  // всю коробку разом, а не одну её стенку.
  bey(sila){ this.din.udar(sila); this.udarT += sila * 9; }
...
class Chaos extends AudioWorkletProcessor {
  constructor(){
    ...
    this.p = { sway:.55, tone:.5, depth:.75,
               ...
               // ударный контур: на нуле его в схеме нет
               bas:0 };
