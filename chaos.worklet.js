...
class Build {
  constructor(semya){
    ...
    this.pezo = [m(.004, .022), m(.004, .022), m(.004, .022)];
...
class Device {
  constructor(semya){
    ...
    // ЗВОН ПЛАТЫ: затухающий резонатор, возбуждаемый ударом по корпусу. На нём
    // сидит керамика, и её пьезоэффект переносит этот звон прямо в узлы.
    this.zvon = 0; this.zvonV = 0; this.pzBylo = 0;
  ...
  step(p, utechka, navodka, kontakt, gls, ogib, petl, pyat, shoroh, rkont){
    ...
    let dPz = 0;
    if (this.zvon !== 0 || this.zvonV !== 0){
      const w = 2 * Math.PI * sb.fPlaty / SR;
      this.zvonV += (-w * w * this.zvon - (w / sb.qPlaty) * this.zvonV);
      this.zvon += this.zvonV;
      // Пьезо даёт заряд от СКОРОСТИ деформации, а не от смещения: сдвиг узла
      // идёт разностью, оттого удар и слышен щелчком, а не гудением.
      dPz = this.zvon - this.pzBylo; this.pzBylo = this.zvon;
      if (Math.abs(this.zvon) < 1e-7 && Math.abs(this.zvonV) < 1e-7){
        this.zvon = 0; this.zvonV = 0; this.pzBylo = 0;
      }
    }
  ...
  // УДАР НИЧЕГО НЕ ДОБАВЛЯЕТ К ЗВУКУ — ОН ВОЗМУЩАЕТ СХЕМУ. Оттого он и не
  // может прозвучать блипом: звучит потом сам прибор, потрясённый.
  bey(sila){
    // ЗВУКА КОРОБКИ ЗДЕСЬ НЕТ ВОВСЕ, и это не упрощение, а суть.
    //
    // Прежде удар тряс капсюль: мембрана шла по инерции относительно рамы, и
    // получался шлепок — тот самый «кик», который к прибору отношения не
    // имеет. Слышать надо не коробку, а как от толчка ведёт САМО
    // ЭЛЕКТРИЧЕСТВО: плата звенит, керамика на ней рождает заряд, узлы
    // сдвигаются, и дальше звучит потрясённая схема.
    //
    // Стук ладони по пластику в комнате слышен и так — микрофоном, если он
    // включён. Внутрь тракта ему хода нет.
    this.zvon += sila * 90;
  }
...
class Contacts {
  constructor(){ this.dreb = 0; this.faza = 0; this.dl = .007;
                 this.shoroh = 0; this.medlenno = 0; this.rasshat = 0; }
  ...
  ryvok(sila){
    // БАТАРЕЯ СДВИГАЕТСЯ В ДЕРЖАТЕЛЕ, и это главное последствие удара.
    //
    // Пружинный дребезг сам по себе — единицы десятков миллисекунд, и удар от
    // него проходит мгновенно. Но крона от хорошего толчка ещё и ПЕРЕКАШИВАЕТ
    // в гнезде: пятно контакта уменьшается, переходное сопротивление растёт в
    // разы, и таким контакт остаётся, пока пружина не дожмёт батарею обратно.
    // Это секунды, а не миллисекунды.
    //
    // Что слышно всё это время: шина стала мягкой, прибор проседает под
    // собственным током, захлёбывается и потрескивает — ровно тот же
    // механизм, что у севшей батареи, только временно и само проходит.
    this.rasshat = Math.min(1, this.rasshat + clamp(sila, 0, 1) * 1.4);
    ...
    // Дребезг пружины после хорошего удара длится не единицы миллисекунд, а
    // десятки: контакт отпускает и ловит по нескольку раз. Замер с контрольным
    // прогоном (тот же прибор, то же состояние, в одном бьём, в другом нет)
    // показал, что коротким отрывом удар не слышен вовсе.
    this.dl = .012 + Math.random() * .072 * clamp(sila, 0, 1);
    return true;
  }
  // множитель питания: во время дребезга оно рвётся
  step(){
    // Расшатанный контакт садится обратно, и всё это время норовит потерять
    // пятно ещё раз. Частота таких потерь падает вместе с расшатанностью —
    // оттого треск и «успокаивается», а не обрывается.
    //
    // ДВЕ РУЧКИ, И ОБЕ БЫЛИ ВЫВЕРНУТЫ. Затухание .99999 давало постоянную в
    // 2.08 с и окно до тишины 14.4 с; частота 6e-3 давала 288 попыток в
    // секунду, то есть треть времени прибор был обесточен. Замер: 30 %
    // скважности на первой секунде, отдельные щелчки до тринадцатой.
    //
    // Слышно это стеной и хвостом не по делу. Пружинный держатель кроны
    // дожимает батарею обратно за доли секунды — ну за пару; четырнадцать
    // секунд перемежающегося контакта означают не расшатанный держатель, а
    // сломанный. Теперь постоянная 0.63 с (окно 4.4 с) и 105 попыток в
    // секунду: рывок, потом отдельные щелчки, потом тишина.
    if (this.rasshat > .001){
      this.rasshat *= .999967;
      if (this.dreb <= 0 && Math.random() < this.rasshat * 2.2e-3){
        this.dreb = 1; this.faza = 0;
        // Обрывы КОРОТКИЕ: доли миллисекунды до трёх. Длинные читаются
        // глотками и складываются в то же качание, от которого уходим.
        this.dl = .0004 + Math.random() * .0025;
      }
    } else this.rasshat = 0;
    if (this.dreb > 0){
      this.faza += 1 / SR;
  ...
  //
  // И ЗАТУХАЕТ ОН МЕДЛЕННО. При вращении пятно контакта успокаивается за
  // миллисекунды — рука ведёт его плавно. После удара движок скачет по слою
  // десятки миллисекунд, и звук возвращается не сразу, а «уплывает» обратно.
  // Оттого у толчка своя постоянная, вшестеро дольше.
  tolchok(sila){
    this.shoroh = Math.min(1, this.shoroh + clamp(sila, 0, 1) * 4.5);
    this.medlenno = 1;
  }
  trenie(){
    this.shoroh *= this.medlenno ? .99985 : .9992;
    if (this.shoroh <= .002) this.medlenno = 0;
...
const TUMBLERY = {gen1:1, gen2:1, gen3:1, mix:1, pit:1};
