...
class Battery {
  ...
  step(I, razvyazka, vkl, set, k){
    ...
    if (vkl && set){
      ...
      const Vravn = this.sb.EMF * this.iznos * k - I * this.sb.Rvn;
...
class Device {
  ...
  step(p, utechka, navodka, kontakt, gls, ogib, petl){
    ...
    const kV = v < .5 ? .12 + v * 1.76 : 1 + (v - .5) * 1.1;
    // РУЧКА НАКОПИТЕЛЯ. Сколько прибор живёт после того, как питание снято, —
    // это ёмкость бака, а не размер батареи: батарея решает, сколько он
    // проработает до посадки, а бак — сколько догорает. Ход в РАЗАХ от
    // номинала сборки, середина — свой: вниз обрыв, вверх долгое умирание.
    const b = clamp(p.bak === undefined ? .5 : p.bak, 0, 1);
    this.bat.bak = Math.pow(18, b - .5);
