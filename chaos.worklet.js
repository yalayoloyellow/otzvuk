...
class Battery {
  ...
  step(I, razvyazka){
    ...
    // STARVE — РЕОСТАТ МЕЖДУ БАТАРЕЕЙ И СХЕМОЙ, и стоит он ровно там, где
    // стоял бы в железе: последовательно с внутренним сопротивлением. Отсюда
    // всё и следует само — просадка становится зависящей от ТОКА: громкое
    // место жрёт больше, шина падает сильнее, пороги триггеров (они заданы
    // долями питания) уезжают, и прибор начинает модулировать сам себя.
    //
    // Это НЕ дубль тумблера SAG: тот снимает конденсатор развязки, то есть
    // открывает логике дорогу к пульсации, которая на шине УЖЕ ЕСТЬ. Реостат
    // эту пульсацию СОЗДАЁТ. Вместе они дают пару «сколько трясётся» и
    // «слышит ли логика тряску».
    const Vravn = this.sb.EMF * this.iznos - I * (this.sb.Rvn + this.Rstarve);
...
class Device {
  ...
  step(p, utechka, navodka, kontakt, gls, ogib, petl){
    ...
    // STARVE считается В АБСОЛЮТНЫХ ОМАХ, а не в долях Rvn.
    //
    // Сначала было в долях — казалось, так ручка одинаково душит любую
    // сборку. Замер это опроверг: просадка равна I·R, а ток от Rvn не
    // зависит вовсе. У сборок с малым внутренним сопротивлением (разброс
    // вдесятеро, 2.5…26 Ом) ручка выходила почти немой: шина падала с 99 до
    // 96 процентов вместо 25. Одинаково душит одинаковое сопротивление.
    //
    // ЗАКОН ХОДА — КВАДРАТ. Шина от него, в процентах от ЭДС:
    //
    //     ход        0.00  0.20  0.35  0.50  0.70  0.85  1.00
    //     квадрат      96    59    34    21    14    12    11
    //     куб          96    93    78    49    22    14    11
    //
    // Куб я выбрал первым, рассудив, что он «раскладывает просадку ровно».
    // Ухо это отвергло, и правильно: при кубе первые две трети хода не делают
    // НИЧЕГО — до слышимого места надо докрутить пятнадцатью нажатиями, и
    // ручка кажется сломанной. Начало хода, честно ничего не меняющее, — это
    // мёртвая зона, а не честность. При квадрате просадка идёт с первого
    // поворота.
    this.bat.Rstarve = 2600 * clamp(p.starve || 0, 0, 1) ** 2;
