...
class Zhmi {
  // Компрессор. Не физика, а инструмент сведения — и только он: уровнем
  // владеет петля громкости, а этот меняет ПЛОТНОСТЬ.
  //
  // Раньше внутри стояла ещё и медленная часть, тянувшая среднюю громкость к
  // отметке. Она была нужна, пока петли не было. Теперь это ВТОРОЙ автомат
  // уровня в той же цепи, и они дерутся: компрессор двигает громкость, петля
  // видит и двигает обратно, потом компрессор снова. На слух это «перегрузил,
  // потом нормализовал, потом опять». Медленная часть убрана.
  //
  // Вместо неё АВТОДОБОР: цепь возвращает ровно столько, сколько придавила,
  // усреднённо. Тогда компрессор нейтрален по громкости по построению —
  // крутишь его, и уровень не шевелится вовсе, меняется только плотность.
  constructor(){ this.og = 0; this.g = 1; this.effekt = 1; }
  step(y, sila){
    if (sila < .002){
      this.g += (1 - this.g) * this.otp;
      this.effekt = this.g;
      return y * this.effekt;
    }
    const a = y < 0 ? -y : y;
    this.og += (a - this.og) * (a > this.og ? this.atk : this.otp);
    // Мягкое колено, считанное в децибелах, как их и считают. Ниже колена не
    // трогаем, выше сжимаем с полным отношением, внутри колена отношение
    // нарастает по квадрату — без колена на пороге слышен щелчок вступления.
    const k = 1 - 1 / (1 + sila * 4);            // 0 → 1:1, 1 → 1:5
    // Ручка ведёт и ПОРОГ: без этого сжимать было почти нечего — среднее
    // придавливание упиралось в два децибела на всём ходу. Порог уходит вниз
    // вместе с отношением, и глубина получается настоящая.
    const nad = 20 * Math.log10(this.og + 1e-9) - (TDB - sila * 14);
    let sniz = 0;
    ...
    // ДОБОР СТАТИЧЕСКИЙ, по кривой, снятой замером с этого материала. Измерять
    // его в реальном времени нельзя: измерение по определению отстаёт, и на
    // это отставание громкость едет — ровно то, что слышно как «перегрузил,
    // потом нормализовал». Число знает, сколько вернуть, ещё до того, как
    // придавит.
    this.effekt = this.g * DOBOR(sila);
    return y * this.effekt;
  }
}
// ДОБОР по кривой, снятой замером с этого материала: среднее придавливание
// компрессора идёт от нуля до 12.3 дБ почти линейно по ручке, с лёгким
// изгибом. Возвращаем ровно столько — и громкость при вращении не шевелится
// вовсе, меняется только плотность.
//   COMP 0.125 → 1.20 дБ · 0.25 → 2.52 · 0.5 → 5.60 · 0.75 → 8.92 · 1 → 12.31
const DOBOR = s => Math.pow(10, (10.09 * s + 2.22 * s * s) / 20);
const TDB = -24;    // порог, дБ относительно полной шкалы
...
class Gromkost {
  ...
  step(y, cel){
    ...
    if (this.videl && this.ms > 1e-9){
      this.lufs = -.691 + 10 * Math.log10(this.ms);
      ...
      const bystro = (promah < 0 ? -promah : promah) > 6;
      // Вниз всегда быстрее, чем вверх: перегруз опаснее тишины. И большой
      // промах догоняется быстрее малого — иначе после Tab новая сборка
      // десять секунд выходила бы на громкость.
      const t = promah < 0 ? (bystro ? .8 : 2.5) : (bystro ? 1.6 : 4.0);
      this.db = clamp(this.db + promah * (1 - Math.exp(-1 / (SR * t))), -24, 24);
...
class Predel {
  ...
  // ОТПУСКАНИЕ — единственное, что в ограничителе можно отдать в руки, и
  // единственное, чего петля громкости не может отменить: оно меняет не
  // уровень, а форму. Быстрое даёт накачку и дыхание, на которых эта музыка
  // и держится; медленное — прозрачность.
  nastroy(rel){
    const t = .005 * Math.pow(60, clamp(rel, 0, 1));   // 5 мс … 300 мс
    this.otp = 1 - Math.exp(-1 / (SR * t));
  }
  step(y, glub){
...
class Chaos extends AudioWorkletProcessor {
  ...
  process(inputs, outputs){
    ...
    for (let s = 0; s < n; s++){
      ...
      // звук поедет.
      // DRIVE ОПУСКАЕТ ПОТОЛОК — и это единственный способ загнать сигнал в
      // ограничитель, когда выходным уровнем владеет петля.
      //
      // Усиление ПЕРЕД петлёй она съедает целиком: поднял вдвое — убавила
      // вдвое. Потолок она съесть не может, потому что это порог ОТНОСИТЕЛЬНО
      // сигнала, а не сигнал. Опускаешь — ограничитель работает больше,
      // петля добирает громкость обратно, и звук выходит плотнее при том же
      // уровне. Добор за ограничителем ставить нельзя: он превращает потолок
      // обратно в усиление, и петля снова всё сокращает — на этом я и
      // ошибся в прошлый раз.
      //
      // Глубина ограничена замером: ниже 0.48 цель −14 LUFS недостижима
      // физически. Сжатие этого материала упирается в крест-фактор 2.29, и
      // при потолке 0.36 средняя громкость выходит −16.9 как ни старайся.
      y = koleno(this.predel.step(y * this.grom.g, Math.pow(.60, this.p.drive)));
      this.predel.nastroy(this.p.rel);
      this.grom.step(y, CEL_LUFS);
      this.proshY = y;
      ...
      if (++this.oscsh >= shago){ this.oscsh = 0;
        ...
        this.oscG[this.osci] = gDo * this.zhmi.effekt * this.grom.g
                             * this.predel.g; }
    ...
    if (this.report >= SR / 30){
      ...
      this.port.postMessage({
        ...
        lufs: this.grom.lufs, lug: this.grom.db,
