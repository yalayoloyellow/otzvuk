...
class Battery {
  ...
  // vkl — замкнут ли выключатель питания.
  step(I, razvyazka, vkl){
...
class Device {
  ...
  step(p, utechka, navodka, kontakt, gls, ogib, petl){
    ...
    const podano = (p.pit || 0) > .5;
    this.bat.step(tok + (this.tokdin || 0), 1 - clamp(p.dirt, 0, 1), podano);
    // Выключенная батарея не садится.
    if (podano) this.bat.sadis();
