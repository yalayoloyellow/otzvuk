...
// Меняет ли палец ВЕСЬ прибор или только свой генератор — зависит от того,
// насколько генераторы связаны между собой. Проверяем при включённом SYNC и
// при трёх работающих генераторах.
for (const [имя, изм] of [['SYNC включён', {link:1}],
                          ['три генератора', {gen3:1}],
                          ['SYNC + три', {link:1, gen3:1}],
                          ['SYNC + три + развод', {link:1, gen3:1, spread:.5}]]){
