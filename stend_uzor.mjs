...
console.log('  семя        узор                                мин/мед окна: без    с узором');
let бед=0, жив=0;
for(const s of семена){
  ...
  const мина=оа[0], меда=оа[оа.length>>1], минб=об[0], медб=об[об.length>>1];
  // Узор жив, если худшее окно просело сильнее, чем у сухого прогона.
  if (минб < мина * .6) жив++;
  console.log('  '+String(s).padStart(10)+'  '+у.padEnd(36).slice(0,36)+'  '+
    String(мина).padStart(4)+'/'+String(меда).padEnd(5)+'  '+String(минб).padStart(4)+'/'+String(медб));
}
console.log('\n  бед: '+бед+' · узор слышен окнами у '+жив+' из '+семена.length);
