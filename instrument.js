...
// РАЗМЕР КАРТИНЫ ПОСТОЯНЕН, как и всё в приборе.
//
// Панель — пятьдесят два знакоместа при любом окне, кегль восемь при любом
// экране; менять под окно одну только картину было непоследовательно, а
// теперь ещё и незачем: раз она умеет уходить за правый край, подгонять её
// ширину под остаток больше не надо. Прибор одного размера, а окно
// показывает столько, сколько показывает.
//
// Двести двенадцать на шестьдесят четыре — ровно тот овал два к одному
// (1022×512 точек), что помещается рядом с панелью в окне 1440.
const KART_SH=212, KART_V=64;
// Когда видно меньше этого, картину не рисуем вовсе: это уже не картина, а
// обрезок, и панель встаёт одна по центру.
...
function pomer(){
  ...
  // ШИРИНА ОКНА НА РАЗМЕР НЕ ВЛИЯЕТ ВОВСЕ. Не хватает места — картина уходит
  // за правый край, а не ужимается: для того вытеснение и заведено.
  //
  // Единственное, что может её уменьшить, — НИЗКОЕ окно: вниз уходить некуда,
  // там легенда. ЧИСЛО СТРОК ЧЁТНОЕ: строка картины — половина модуля, и
  // только при чётном их числе низ картины садится на ту же линию, что и
  // строки панели.
  let novv=KART_V, nov=KART_SH;
  if(poVys<novv){
    novv=Math.max(10, poVys-(poVys&1));
