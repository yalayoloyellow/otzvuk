...
//
// У АКЦЕНТОВ РЯД СВОЙ, ЖИРНЕЕ НА СТУПЕНЬ. Это не подгонка, а поправка на
// зрение: цветовое разрешение глаза втрое-вчетверо ниже яркостного, и мелкий
// цветной знак усредняется по клетке почти в чёрное. Замер среднего цвета
// клетки для #ff7a5c: точка · даёт rgb(15,7,6), а плотная звезда ※ —
// rgb(135,65,49). Тот же самый цвет, разница вдевятеро. Поэтому красное и
// синее набирают площадь там, где зелёному хватает точки.
//
// Доли клетки картины, замерено отрисовкой в том же шрифте:
//
//     ·  6.0     ◦ 10.9     ∙ 16.8     ✳ 30.9     ※ 53.0     ● 67.0
//
const ZNAKI=[' ·◦∙✳※',    // схема: весь ряд, от точки до плотной звезды
             ' ◦∙✳※●',    // пост:  на ступень жирнее
             ' ◦∙✳※●'];   // голос: так же
const KOD=ZNAKI.map(r=>[...r].map(c=>c.charCodeAt(0)));
// Те же знаки идут и на панель: шкала набрана плотной звездой, погасшее —
// точкой. Панель и картина должны быть одной графики, иначе половины экрана
// выглядят из разных приборов.
const ZN_GORIT='※', ZN_TUHO='·', ZN_SHAG='●';
...
// Цвет у акцентов один на четыре нижние ступени, а ореол свой у каждой.
...
function shkala(v, sh){
  ...
  return ZN_GORIT.repeat(n)+ZN_TUHO.repeat(sh-n);
}
...
function shkalaHTML(v, sh, zona){
  const n=clamp(Math.round(v*sh),0,sh);
  return `<span class="${KL_GORIT[zona]}">${ZN_GORIT.repeat(n)}</span>`+
         `<span class="z0">${ZN_TUHO.repeat(sh-n)}</span>`;
}
function ruchki(){
  ...
  if(ris.length){
    ...
    const s=ris.map((v,i)=> i===shag ? ZN_SHAG : (v?ZN_GORIT:ZN_TUHO)).join('');
  ...
  for(const zn of ZONY){
    ...
    if(tm.length) stroki.push(tm.map(t=>{
      ...
      const vid = pol>2 ? (t.podpis[z]||String(z)) : (z?ZN_GORIT:ZN_TUHO);
