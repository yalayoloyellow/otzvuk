...
// на каждом её гребне.
let svoyaLestnica=false;   // на своей лестнице панель или на пришедшей
const GIST_STUP=.8;
  ...
  for(const s of SHKALY){
    ...
    for(let i=0;i<s.sh;i++){
      ...
      if(d){
        ...
        // ВОЛНЫ ВДОЛЬ ШКАЛЫ БОЛЬШЕ НЕТ. Она бежала по знакоместам и меняла
        // их ступень поодиночке — соседи дышали вразнобой, и хозяин прочёл
        // это как поломанное мерцание: «мерцание на шкалах». Дыхание
        // остаётся, но общее: вся шкала светлеет и темнеет ЦЕЛИКОМ, знаки
        // не перескакивают друг относительно друга, и рябить нечему.
        // Дыхание вернулось: с неподвижной лестницей оно двигает ВСЮ шкалу
        // разом и читается как жизнь, а не как рябь.
        T=4-d/SPAD+dyh;
