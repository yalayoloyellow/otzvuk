...
function ppole(){
  ...
  let L=[L0,GRANI[3],GRANI[2],GRANI[1],GRANI[0]];
  ...
  const узка = L[4] <= L[0] * (svoyaLestnica ? 4 : 3);
  svoyaLestnica = узка;
  if(узка) L=[L0, L0*1.7, L0*2.9, L0*4.9, L0*8.4];
  ...
  for(const s of SHKALY){
    ...
    for(let i=0;i<s.sh;i++){
      ...
      if(d){
        ...
        // Размах волны меньше половины ступени: она должна ЧИТАТЬСЯ как
        // дыхание вдоль шкалы, а не перебрасывать знак через ступень.
        T=4-d/SPAD+dyh+Math.sin(i*.55+s.y*2.7+ugol*2.2)*.22;
