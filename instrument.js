...
const SWITCHES=[
  ...
  // BRIDGE — ошибочная перемычка на плате: выход одной микросхемы заходит
  // резистором прямо в частотозадающий конденсатор соседней. Ведомый
  // захватывается в целые отношения и ходит между ними ступенями.
  //
  ...
  // Замер по трём сборкам показывал рост остроты модуляции вдвое-втрое. Мост
  // петли не образует вовсе — он односторонний, ведущий о ведомом не знает.
  //
  ...
  {k:'sboy', kl:'Space', imya:'BRIDGE', gr:'pit'},
  // ---- СХЕМА ----
...
function ruchki(){
  ...
  for(const imya of ['POWER','MAINS','VOLTS','TANK','SAG','BRIDGE']){
