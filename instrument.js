...
const SWITCHES=[
  ...
  // ПИТАНИЕ. Выключателя у прибора не было вовсе — он начинал играть сам,
  // едва открыв страницу. У всякой коробки он есть, и это не удобство, а
  // орган: выключение не обрывает звук, а разряжает ёмкость по питанию, и
  // прибор оседает по громкости и по высоте разом.
  //
  // Ему отдана ПРОСТАЯ клавиша, а SAG уехал на второй слой: питание жмут
  // каждый раз, а развязку снимают изредка.
  {k:'pit',  kl:'KeyB', imya:'POWER', gr:'верх'},
  // SAG — снятие развязки питания: шина проседает, и логика слышит сама себя.
  {k:'dirt', kl:'KeyB', imya:'SAG',   gr:'setka', shift:1},
...
const switches={pit:0, gen1:1, gen2:1, gen3:0, link:0, dirt:0, petlya:0,
                mix:0, povtor:0};
...
function ruchki(){
  ...
  // ПИТАНИЕ СТОИТ ПЕРВЫМ И ОТДЕЛЬНО. Это не одна из ручек схемы, а то, с чего
  // прибор начинается: пока выключатель разомкнут, всё остальное на панели
  // ничего не значит. Оттого и своя строка наверху, отбитая пустой.
  stroki.push(tumbler(SWITCHES.find(t=>t.k==='pit'), zs));
  stroki.push('');
