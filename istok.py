...
def подрежь(x, sr, bpm, тактов=2):
    ...
    окно = max(1, min(доля, len(x) - n))
    e = np.abs(x[:окно + 1])
    старт = 0
    if окно > 64:
        шаг = max(1, окно // 128)
        # огибающая по окошкам, старт там, где сильнее всего нарастание
        блоки = np.array([e[i:i + шаг].mean() for i in range(0, окно - шаг, шаг)])
        if len(блоки) > 2:
            рост = np.diff(блоки)
            старт = int((int(np.argmax(рост)) + 1) * шаг)
