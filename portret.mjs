...
//
// Зачем. Правку звука проверяет только ухо, и каждая непроверенная догадка
// стоит человеку прослушивания. Четыре подряд это и показали.
//
// Отсюда картинка: из тех же отсчётов строится изображение, на котором видно
// ровно то, что слышно уху. Не инженерный график, а перцептивный портрет.
//
...
//
// И тот же прогон пишется в WAV: человек слушает ровно то, на что смотрит
// машина. Не «похожее», а те же самые отсчёты — иначе разговор опять пойдёт
// о разных вещах.
//
...
const БАЗА = {sway:.55,depth:.75,gryzn:0,golos:0,petlya:0,kuda:0,zhat:0,drive:.15,master:1,pit:1,sboy:0};
const SR = 48000;
...
//  ЧТЕНИЕ ЖИВОЙ ЗАПИСИ.
//
//  Смысл один: связать услышанное с числами. Не словарь явлений, не эталоны
//  и не приговоры — человек показывает на кусок звука, и у этого куска есть
//  измеримые признаки.
//
