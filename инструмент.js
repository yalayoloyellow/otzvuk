...
function картина(){
  ...
  for(let i=0;i<n-сдв;i++){
    ...
    const x=Math.round(цx + (wx+муть)*цx*.94*рx);
    const y=Math.round(цy - (wy+муть)*цy*.94*рy);
...
function ручки(){
  ...
  const строки=[`  <span class="совсем">прибор</span> <b>${сб.имя||'····'}</b>`+
                (сб.излучатель?`  <span class="совсем">динамик ${сб.излучатель.toFixed(0)} Гц ·`+
                 ` фронт ${(сб.фронт*100).toFixed(0)}% · срез ${(сб.срез/1000).toFixed(1)}к</span>`:'')+
